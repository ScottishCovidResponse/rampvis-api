...
function generateTitle(keywordsList:any): string {
    const locs = [];
    const times = [];
    const topics = [];
    const groups = [];
    ...
    // Multiple streams
    return combineToTitle(maxLoc(locs), sameKeyword(times), sameKeyword(topics), sameKeyword(groups), sameKeyword(types), sameKeyword(models));
}

function combineToTitle(loc:string|null, time:string|null, topic:string|null, group:string|null, type:string|null, model:string|null) {
    if (topic === null) {
