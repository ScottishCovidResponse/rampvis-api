...
export class BookmarkService extends DataService<IBookmark> {
    public constructor(@inject(TYPES.DbClient) dbClient: DbClient) {
        super(dbClient, config.get('mongodb.db'), config.get('mongodb.collection.bookmarks'));
    }
    ...
    async deleteBookmark(user: IUser, pageId: string): Promise<IBookmark> {
        const res: FindAndModifyWriteOpResultObject<IBookmark> = await this.getDbCollection().findOneAndDelete({
            userId: <string>user._id,
            pageId: pageId,
