...
export class OntoPageService extends DataService<IOntoPage> {
  ...
  private async getPageBindingVisIdAndDataIds(_visId: string, _dataIds: string[]): Promise<IOntoPage[]> {
    const pages = await this.getAll({ visId: _visId, dataIds: { $in: _dataIds } });
    return pages;
  }
  ...
  public async updateChildrenPageIds(pageId: string, childrenPageIds: string[]): Promise<any> {
    return this.getDbCollection().updateOne(
      { _id: new ObjectId(pageId) },
      { $set: { childrenPageIds: childrenPageIds } },
      { upsert: false }
    );
  }

  public async updateParentPageId(pageId: string, parentPageId: string): Promise<any> {
    return this.getDbCollection().updateOne(
      { _id: new ObjectId(pageId) },
  ...
  public async updateNeighborPageIds(pageId: string, neighborPageIds: string[]): Promise<any> {
    return this.getDbCollection().updateOne(
      { _id: new ObjectId(pageId) },
      { $set: { neighborPageIds: neighborPageIds } },
      { upsert: false }
    );
  }
}
